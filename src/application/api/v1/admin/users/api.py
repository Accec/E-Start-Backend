...
@openapi.parameter(name="account", required=False, schema={"endpoint": "str"})
...
async def admin_post_users(request: Request, body: serializers.AdminPostUsersBody):
    ...
    new_log = Log(user = user, api = request.uri_template, action = f"New User [{body.account}] is created", ip = request.ctx.real_ip, ua = request.ctx.ua, level = LOG_LEVEL_MIDIUM)
    await new_log.save()
...
async def admin_put_users(request: Request, body: serializers.AdminPutUsersBody):
    ...
    new_log = Log(user = user, api = request.uri_template, action = f"Users [{await old_user_model.values()}] is change to [{await user_model.values()}]", ip = request.ctx.real_ip, ua = request.ctx.ua, level = LOG_LEVEL_MIDIUM)
    await new_log.save()
...
async def admin_post_users_roles(request: Request, body: serializers.AdminPostUsersRolesBody):
    ...
    new_log = Log(user = user, api = request.uri_template, action = f"User [{users_model.account}] add Role [{roles_model.role_name}]", ip = request.ctx.real_ip, ua = request.ctx.ua, level = LOG_LEVEL_MIDIUM)
    await new_log.save()
...
@JwtAuth.permissions_authorized()
async def admin_delete_endpoints_permissions(request: Request, body: serializers.AdminDeleteUsersRolesBody):
    if body.id and body.account:
        users_model = User.filter(id=body.id, role_name=body.account)
    elif body.id:
        users_model = User.filter(id=body.id)
    ...
    new_log = Log(user = user, api = request.uri_template, action = f"User [{users_model.account}] remove Role [{roles_model.role_name}]", ip = request.ctx.real_ip, ua = request.ctx.ua, level = LOG_LEVEL_MIDIUM)
    await new_log.save()
